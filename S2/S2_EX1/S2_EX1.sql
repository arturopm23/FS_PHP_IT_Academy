...
/*11*/ SELECT f.codigo, f.nombre FROM fabricante f JOIN producto p ON (f.codigo = p.codigo_fabricante);
/*12*/ SELECT DISTINCT f.codigo, f.nombre FROM fabricante f JOIN producto p ON (f.codigo = p.codigo_fabricante);
...
/*25*/ SELECT p.nombre AS producto , p.precio, f.nombre AS fabricante FROM producto p JOIN fabricante f ON (f.codigo = p.codigo_fabricante) ORDER BY p.precio LIMIT 1;
...
/*28*/ SELECT p.nombre AS producto , p.precio, f.nombre AS fabricante FROM producto p JOIN fabricante f ON (f.codigo = p.codigo_fabricante) WHERE p.codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Asus') OR p.codigo_fabricante =  (SELECT codigo FROM fabricante WHERE nombre = 'Hewlett-Packard') OR p.codigo_fabricante =  (SELECT codigo FROM fabricante WHERE nombre = 'Seagate');
/*29*/ SELECT p.nombre AS producto , p.precio, f.nombre AS fabricante FROM producto p JOIN fabricante f ON (f.codigo = p.codigo_fabricante) WHERE p.codigo_fabricante IN ((SELECT codigo FROM fabricante WHERE nombre = 'Asus'), (SELECT codigo FROM fabricante WHERE nombre = 'Hewlett-Packard'), (SELECT codigo FROM fabricante WHERE nombre = 'Seagate'));
...
/*1*/ SELECT pe.nombre, pe.apellido1, pe.apellido2, d.nombre AS departamento FROM persona pe JOIN profesor pr ON (pe.id = pr.id_profesor) LEFT JOIN departamento d ON (pr.id_departamento = d.id) WHERE tipo = 'profesor' ORDER BY d.nombre, pe.apellido1, pe.apellido2, pe.nombre;
/*2*/ SELECT * FROM persona pe JOIN profesor pr ON (pe.id = pr.id_profesor) LEFT JOIN departamento d ON (pr.id_departamento = d.id) WHERE tipo = 'profesor' AND d.nombre IS NULL; 
/*3*/ SELECT DISTINCT * FROM departamento de LEFT JOIN profesor p ON (de.id = p.id_departamento) WHERE p.id_profesor IS NULL;
/*4*/ SELECT pe.nombre, pe.apellido1, pe.apellido2, asi.nombre AS asignatura FROM persona pe JOIN profesor pr ON (pe.id = pr.id_profesor) LEFT JOIN asignatura asi ON (asi.id_profesor = pr.id_profesor) WHERE asi.nombre IS NULL; 
...
/*3*/ SELECT d.nombre, COUNT(*) AS profesores FROM persona pe JOIN profesor pr ON (pe.id = pr.id_profesor) LEFT JOIN departamento d ON (pr.id_departamento = d.id) GROUP BY d.nombre;
...
/*8*/ SELECT ce.anyo_inicio, COUNT(asma.id_alumno) AS alumnos FROM asignatura a JOIN alumno_se_matricula_asignatura asma ON (asma.id_asignatura =a.id) JOIN curso_escolar ce ON (asma.id_curso_escolar = ce.id) GROUP BY asma.id_curso_escolar;
/*9*/ SELECT pe.id, pe.nombre, pe.apellido1, pe.apellido2, COUNT(asi.id) AS asignaturas FROM profesor pr JOIN persona pe ON (pr.id_profesor = pe.id) LEFT JOIN asignatura asi ON (asi.id_profesor = pr.id_profesor) GROUP BY pr.id_profesor;
